...
def load_args():
    ...
    parser.add_argument('--brute', default=False, type=bool)
...
def find_widths_brute(file, keep_atts, edges):
    graph = create_bayes_net(file, keep_atts, edges)
    N = len(keep_atts)
    perms = list(itertools.permutations(keep_atts))
    ATT_DICT = { keep_atts[i] : i+1 for i in range(0, len(keep_atts) ) }
    ...
    for perm in tqdm.tqdm(perms):
        ...
        runtime = 0
        start_time = time.time()
        iwidth = inference.induced_width(list(perm))
        end_time = time.time()
        runtime = end_time-start_time
        total_sat_runtime += runtime
        max_widths.append(iwidth)
        cliques.append(curr_clique)
        dictionaries.append(curr_dict)
        sat_runtimes.append(runtime)
    return np.array(max_widths), cliques, dictionaries, sat_runtimes, total_sat_runtime
...
def main_brute(file, keep_atts, edges):
    ...
    max_widths, cliques, dictionaries, sat_runtimes, total_sat_runtime = find_widths_brute(file, keep_atts, edges)
    min_tree_width_idx = np.argmin(max_widths)
    min_tree_width = np.min(max_widths)
    best_ordering = perms[min_tree_width_idx]
    best_clique = cliques[min_tree_width_idx]
...
def brute(file):
    ...
    sat_runtimes = []
    for i in range(len(utils.EXPERIMENTAL_ATTS)):
        graph_size, density, n_edges, worst_runtime, total_runtime = main_brute(file, utils.EXPERIMENTAL_ATTS[i], utils.EXPERIMENTAL_EDGES[i])
        g_sizes.append(graph_size)
        sat_runtimes.append(total_runtime)

    plt.title('Brute Runtime vs Number of Random Variables')
    plt.plot(g_sizes, sat_runtimes, 'r-.')
    plt.xlabel('Number of Variables')
    plt.ylabel('Brute Runtime (sec)')
    plt.savefig('./figures/runtime_brute.png')
    plt.close('all')

    plt.title('Brute Runtime vs Number of Random Variables (logscale)')
    plt.plot(np.log(g_sizes), np.log(sat_runtimes), 'g-.')
    plt.xlabel('Number of Variables')
    plt.ylabel('Brute Runtime (sec)')
    plt.savefig('./figures/runtime_log_brute.png')
    plt.close('all')
...
if __name__=='__main__':
    ...
    if args.experiments:
        ...
        exit(0)
    if args.brute:
        print('Brute on Celebrity Data', end='\n\n')
        file = './data/list_attr_celeba.csv'
        brute(file)
        exit(0)
